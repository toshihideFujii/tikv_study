...
impl EventIterator<'_> {
  ...
  pub fn with_rewriting<'a>(buf: &'a [u8], from: &'a [u8],
    to: &'a [u8]) -> EventIterator<'a>
  {
    EventIterator {
      buf: buf,
      offset: 0,
      ...
      value_len: 0,
      key_buf: vec![],
      rewrite_rule: Some(Rewrite { from: from, to: to })
    }    
  }
  ...
  fn consume_key_with_len(&mut self, key_len: usize) {
    ...
    self.key_buf.extend_from_slice(&self.buf[self.offset..self.offset + key_len]);
    self.offset += key_len;
  }
  ...
  fn move_to_next_key_with_rewrite(&mut self) {
    ...
    self.key_buf.clear();
    self.key_buf.reserve(rewrite.to.len() + key_len - rewrite.from.len());
    self.key_buf.extend_from_slice(rewrite.to);
    self.key_buf.extend_from_slice(&self.buf[self.offset + rewrite.from.len()..self.offset + key_len]);
    self.offset += key_len;
  }
