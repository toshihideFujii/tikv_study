...
impl KeyBuilder {
  ...
  pub fn from_vec(mut vec: Vec<u8>, reserved_prefix_len: usize,
    reserved_suffix_len: usize) -> Self
  {
  ...
  pub fn from_slice(s: &[u8], reserved_prefix_len: usize,
    reserved_suffix_len: usize) -> Self
  {
    let mut buf =
        Vec::with_capacity(s.len() + reserved_prefix_len + reserved_suffix_len);
    if reserved_prefix_len > 0 {
      unsafe { buf.set_len(reserved_prefix_len) }
    }
  ...
  pub fn set_prefix(&mut self, prefix: &[u8]) {
    ...
    unsafe {
      ptr::copy_nonoverlapping(prefix.as_ptr(), 
        self.buf.as_mut_ptr(), prefix.len());
      self.start = 0;
    }
  }
...
mod tests {
  ...
  fn test_key_builder() {
    ...
    let mut key_builder = KeyBuilder::new(
        key1.len() + prefix.len() + suffix.len(),
        prefix.len());
    ...
    let key2 = b"key2";
    let mut key_builder = KeyBuilder::from_vec(
      key2.to_vec(),
      prefix.len(),
      suffix.len());
    ...
    let key3 = b"key3";
    let mut key_builder = KeyBuilder::from_slice(
      key3,
      prefix.len(),
      suffix.len());
