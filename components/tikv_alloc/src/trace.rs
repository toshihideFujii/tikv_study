...
impl Add for TraceEvent {
  ...
  fn add(self, other: Self) -> Self {
    match (self, other) {
      ...
      (TraceEvent::Add(l), TraceEvent::Sub(r))
      | (TraceEvent::Sub(r), TraceEvent::Add(l)) => {
        if l > r {
          TraceEvent::Add(l - r)
        } else {
          TraceEvent::Sub(r - l)
...
impl MemoryTrace {
  ...
  pub fn trace_guard<T: Default>(
    ...
    MemoryTraceGuard { item: item, size: size, node: node }
  }
...
mod tests {
  ...
  fn test_trace_event_add() {
    ...
    assert_eq!(TraceEvent::Add(1) + TraceEvent::Reset(3), TraceEvent::Reset(3));
    assert_eq!(TraceEvent::Sub(1) + TraceEvent::Reset(3), TraceEvent::Reset(3));
    assert_eq!(TraceEvent::Reset(3) + TraceEvent::Add(1), TraceEvent::Reset(4));
    assert_eq!(TraceEvent::Reset(3) + TraceEvent::Sub(1), TraceEvent::Reset(2));
  }
}